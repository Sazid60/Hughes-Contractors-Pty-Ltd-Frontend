...
export default function SuccessSection() {
    ...
    const startYear = 2000; // Change this to the actual founding year
    const currentYear = new Date().getFullYear();
    const yearsInIndustry = currentYear - startYear;

    const stats = [
        { icon: FaAward, number: yearsInIndustry, suffix: "+", label: "Years in Industry" },
        { icon: FiCheckCircle, number: isLoading ? 0 : totalProjects, suffix: "+", label: "Total Projects" },
    ];
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:col-span-1">
                        {stats.map((stat, index) => {
                            ...
                            return (
                                <Card
                                    key={index}
                                    className="bg-transparent border-none flex flex-col justify-center items-center rounded-none w-full shadow-none"
                                >
                                    <CardContent className="p-3 text-center flex flex-col justify-center items-center h-full">
                                        <div className="flex justify-center mb-4">
                                            <Icon className="w-20 h-20 text-white" />
                                        </div>
                                        <div className="text-2xl sm:text-6xl font-bold mb-1 text-white">
